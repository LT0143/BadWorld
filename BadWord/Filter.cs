    ...
    public class FilterWord
    {
        ...
        private void LoadDictionary()
        {
            if (DictionaryPath != string.Empty)
            {
                ...
                string[] words = System.IO.File.ReadAllLines(DictionaryPath, System.Text.Encoding.Default);     //将敏感字文件读取出来
                foreach (string word in words)
                {
                    string key = this.ToDBC(word).Replace(" ", "");      //全角转半角?
                    if (key != string.Empty)
                    wordList.Add(key);
                ...
                {
                    if (wordList[i].ToString() == wordList[i - 1].ToString())     //检查有没有重复
                    {
                        wordList.RemoveAt(i);
                    }
                }
                ...
                {
                    WordGroup group = MEMORYLEXICON[(int)word[0]];　　　//首字母分类
                    if (group == null)
                    {
                        group = new WordGroup();
                ...
                else
                {
                    ...
                    for (int y = nextCursor; y < sourctText.Length; y++)
                    {
                        if (!isCHS(sourctText[y]) && !isNum(sourctText[y]) && !isAlphabet(sourctText[y]))
                        {
                            ...
                            wordlenght++;  //提取出检查的单词数字字母
                        }
                        else break;
                    }

                    //if ((int)blackWord[i] == (int)sourctText[nextCursor + offset])
                    if (sourctText.Contains(blackWord))
                    {
                        found = true;
                    }
                    else
